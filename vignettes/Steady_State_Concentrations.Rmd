...
vignette: >
  %\VignetteIndexEntry{Steady_State_Concentrations}
...
![The opening interface to the ToCS app for example 1.](GUI_Introduction_GenParsPage.png){width="100%"}

<br/><br/>

## Example 1

In this example, let's say we want to generate human steady state concentrations in whole body plasma for a constant daily dose of 1 mg/kg BW using the PBTK model with in silico generated parameters available. We do not have specific chemicals in mind, so we will just select chemicals from the preloaded list of chemicals.

### General Parameters Tab

Under the *Output* card, we select *Steady state concentrations* since that is the type of simulation we want to run. Under the *Species* card, we select *Human* for the first drop down menu and *No* for the second drop down menu since we want human steady state concentrations and are not running animal simulation. As with the *Concentration-Time Profile* vignette, we could have selected *No* for the second drop down menu under the *Species* card and it would not make a difference in the simulation results. The completed *General Parameters* card is shown in the image below.

<br/><br/>

![The completed General Parameters tab for example 1.](GUI_SS_GenParsEx1.png){width="100%"}

<br/><br/>

### Model Specifications Tab

Under the *Dosing* card, we leave the total daily dose as its default value of 1 mg/kg BW. Under the *Model* card, we select *pbtk* under the first drop down to use the pbtk model. For the second drop down, we select *Yes* since we want to include compounds with only in silico generated parameters into the selection availability on the next page. Thus, the completed model specifications tab should look like the image below.

<br/><br/>

![The completed model specifications tab for example 1.](GUI_SS_ModelSpecEx1.png){width="100%"}

<br/><br/>

### Compound Selection Tab

Since we selected to load in silico generated parameters, the *Preloaded Compounds* card will take a few moments to load. Once loaded, we search through the list and select 10 compounds:
...
We leave the *Uploaded Compounds* card as is, and click on the *Load Compounds* button under the *Instructions* card to continue onto the next tab. The completed *Compound Selection* tab should look like the image below.
...
![The completed compound selection tab for example 1.](GUI_SS_CompSelectionEx1.png){width="100%"}
...
![The edited Advanced Parameters tab for example 1.](GUI_SS_AdvParsEx1.png){width="100%"}
...
![The completed compound selection tab for example 1 showing all selected compounds.](GUI_SS_RunSimEx1_1.png){width="100%"}
...
![The run simulation tab with the steady state concentration plot results for example 1.](GUI_SS_RunSimEx1_2.png){width="100%"}
...
![The run simulation tab with the steady state concentration plot results with log10 y-scale for example 1.](GUI_SS_RunSimEx1_3.png){width="100%"}
...
![The run simulation tab with the steady state concentrations table results for example 1.](GUI_SS_RunSimEx1_4.png){width="100%"}

<br/><br/>

The final drop down tab under the *Results* card contains a table of steady state characteristics for each of the compounds. The simulation that generates this table determines the number of days it takes for the compound to come within a certain percentage of analytical steady state value from the table in the tab above (CssDay column), the average concentration on the last day of the simulation (AvgConc column), the ratio of the average concentration to the analytical steady state concentration (RatioAvgAnalytical), and the maximal concentration of the simulation (MaxConc column). The user can download this table by clicking the *Download Table 2* button at the top of the tab. The table should look like that in the image below.

<br/><br/>

![The run simulation tab with the days to steady state table results for example 1.](GUI_SS_RunSimEx1_5.png){width="100%"}

<br/><br/>

If the user want to run a new simulation, we suggest clicking the *Reset Session* button under the *Actions* card, which will reset all parameter values and return the user to the starting interface.

## Example 2

In this example, let's say we want to generate human steady state concentrations in liver blood for a constant daily dose of 2 mg/kg BW using the 1compartment model without in silico generated parameters. Suppose we want to generate these concentrations for the following compounds: Valproic Acid (CAS: 99-66-1), Endosulfan (CAS: 115-29-7), Abamectin (CAS: 71751-41-2), Titanium Dioxide (CAS: 13463-67-7), and Advantame (CAS: 714229-20-6).
...
library(knitr)
library(kableExtra)
SampleChemsToUpload <- read.csv("CSV_SSvignette.csv")
...
Once we have the three compounds selected under the *Preloaded Compounds* card and the two compounds uploaded under the *Uploaded Compounds* card (with the csv file name CSV_SSvignette.csv), we then hit the *Load Compounds* button and proceed to the next tab. The final *Compound Selection* tab should look like the image below.

<br/><br/>

![The completed compound selection tab for example 2.](GUI_SS_CompSelectionEx2.png){width="100%"}

<br/><br/>

### Advanced (Optional) Parameters Tab

Since we want the steady state concentrations in the liver blood, we customize the second and third drop down menus under the *Output Specification* cards to be *blood* and *liver*, respectively. There are no other customizations we want to make on this page, so the final *Advanced Parameters* page should look like the image below.

<br/><br/>

![The completed advanced parameters tab for example 2.](GUI_SS_AdvParsEx2.png){width="100%"}
...
![The steady state concentration plot with a linear axis for example 2.](GUI_SS_RunSimEx2_2.png){width="100%"}
...
![The steady state concentration plot with a log10 y-axis for example 2.](GUI_SS_RunSimEx2_3.png){width="100%"}
...
![The steady state concentrations table for example 2.](GUI_SS_RunSimEx2_4.png){width="100%"}
...
![The days to steady state table for example 2.](GUI_SS_RunSimEx2_5.png){width="100%"}
